  ...
  const fetchActiveAddressName = useCallback(
    async (m: LocationBasedMerchant): Promise<string | null> => {
      ...
      try {
        ...
        console.warn("NewlyUpdated page: merchant detail fetch failed, trying coords fallback:", e);
      }
      ...
      if (lat != null && lng != null) {
        try {
          ...
          console.warn("NewlyUpdated page: address coords lookup failed:", e);
        }
      }
      return null;
    },
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const sorted = (data || []).slice().sort((a, b) => getUpdatedTimeMs(b) - getUpdatedTimeMs(a));
        setMerchants(sorted);
