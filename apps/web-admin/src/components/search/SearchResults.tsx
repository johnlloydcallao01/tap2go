...
import { Search, Clock, X, Store, Package, ShoppingBag, Users, Truck } from '@/components/ui/IconWrapper';
...
const CATEGORY_COLORS: Record<SearchCategory, string> = {
  ...
  orders: 'bg-purple-50 text-purple-700',
  customers: 'bg-orange-50 text-orange-700',
  drivers: 'bg-teal-50 text-teal-700',
  vendors: 'bg-red-50 text-red-700',
};

function ThumbnailOrIcon({
  thumbnail,
  title,
  type,
}: {
  ...
  if (thumbnail) {
    return (
      <img
        src={thumbnail}
        alt={title}
        className="h-8 w-8 rounded-md object-cover border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#171717]"
      />
    );
  }

  return (
    <span className={`flex-shrink-0 p-1 rounded ${CATEGORY_COLORS[type]}`}>
      {CATEGORY_ICONS[type]}
    </span>
  );
}
...
<div className="font-medium text-gray-900 dark:text-white truncate">{suggestion.label}</div>
      ...
      {!isLoading && query && suggestions.length === 0 && results.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p className="text-xs text-gray-400 dark:text-[#a1a1aa] mt-1">Try searching for merchants, products, orders, or customers</p>
        </div>
      )}
