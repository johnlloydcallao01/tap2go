...
function adminLevelBadge(level?: string): { label: string; cls: string } {
  switch (level) {
  case 'system':
  return { label: 'System Admin', cls: 'bg-gradient-to-r from-amber-500 to-orange-600 text-white' };
  case 'department':
  return { label: 'Department Admin', cls: 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white' };
  case 'content':
  return { label: 'Content Admin', cls: 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white' };
  default:
  return { label: 'Admin', cls: 'bg-slate-800 text-white' };
  }
}
...
// ---------- tab definition ----------
type TabId = 'overview' | 'personal' | 'security' | 'settings';
...
function ProfileInner() {
  ...
  tabParam && (['overview', 'personal', 'security', 'settings'] as TabId[]).includes(tabParam as TabId)
  ? (tabParam as TabId)
  : 'overview';
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('admin:profile:prefs');
      if (raw) {
  ...
  setPrefs(next);
  try {
  localStorage.setItem('admin:profile:prefs', JSON.stringify(next));
  } catch {}
  };
  ...
  {displayUser.isActive !== false && <span title="Verified admin"><BadgeCheck className="w-5 h-5 text-sky-500 flex-shrink-0" /></span>}
  </div>
  <div className="flex flex-wrap items-center gap-2 mt-2">
  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold tracking-wide ${adminBadge.cls}`}>
  <Crown className="w-3.5 h-3.5" /> {adminBadge.label}
  </span>
  ...
  <p className="text-xs text-gray-500 dark:text-[#a1a1aa] dark:text-[#a1a1aa] mt-3">Your last successful authentication to the admin panel.</p>
  </div>
  <div className="bg-white dark:bg-[#171717] border border-gray-200 dark:border-[#262626] rounded-xl p-4">
  <div className="flex items-center gap-3">
  ...
  <p className="text-sm font-bold text-gray-900 dark:text-[#ededed]">Managed</p>
  </div>
  </div>
  <p className="text-xs text-gray-500 dark:text-[#a1a1aa] dark:text-[#a1a1aa] mt-3">
  {rawUser?.lockUntil && new Date(rawUser.lockUntil) > new Date() ? (
  <span className="text-red-600 dark:text-red-400 font-medium">Locked until {formatDateTime(rawUser.lockUntil)}</span>
  ) : (
  <span>2FA ready • Session 30d</span>
  )}
  </p>
  </div>
  </div>
  ...
  {adminRecord
  ? adminRecord.adminLevel === 'system'
  ? 'Full platform control — users, payments, delivery & system config.'
  : adminRecord.adminLevel === 'department'
  ? 'Department scoped — operations and merchant oversight.'
  : 'Content focused — catalog, media and communications.'
  ...
  <p className="opacity-80 mt-0.5">Role, ID, isActive are managed by system admins and cannot be edited here.</p>
  </div>
  </div>
  </div>
  ...
  <p className="text-sm text-red-700/80 mt-2 leading-relaxed">Deactivating your admin account will revoke access immediately. Contact system admin — self-deactivation is disabled.</p>
  ...
  <p className="text-sm text-gray-500 dark:text-[#a1a1aa] dark:text-[#a1a1aa] mt-1">Tailor the admin experience. Preferences are stored locally per browser.</p>
  </div>
  ...
  <Info className="w-4 h-4 flex-shrink-0" /> Server-side notification preferences will sync once Communications &gt; User Notifications exposes per-admin settings.
  </div>
  </div>
  </div>
  </div>
  ...
  <a href="/admins" className="flex items-center justify-between p-3 rounded-xl border border-gray-200 dark:border-[#262626] hover:bg-gray-50 dark:hover:bg-[#262626] dark:bg-[#171717] dark:bg-[#262626] text-sm font-medium text-gray-900 dark:text-[#ededed] hover:text-gray-900 dark:text-[#ededed] dark:hover:text-gray-100 dark:text-[#ededed]">
  Admin accounts <span className="text-gray-400 dark:text-[#a1a1aa]">→</span>
  </a>
  </div>
  </div>
