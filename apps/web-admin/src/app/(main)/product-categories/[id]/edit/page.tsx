...
function EditProductCategorySkeleton(){
...
function EditProductCategoryContent() {
  ...
  const handleSaveSuccess = async () => {
    setSaveSuccess(true)
    // Keep the list fresh for when the user navigates back to /product-categories.
  ...
  if (error || !doc) {
    return (
      <div className="space-y-6 py-5 px-2.5">
        <Link href="/product-categories" className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-[#a1a1aa] hover:text-gray-900"><ArrowLeft className="w-4 h-4" /> Back to product categories</Link>
  ...
  return (
    <div className="space-y-6 py-5 px-2.5">
      ...
      {saveSuccess && <div className="flex items-center gap-2 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-sm font-medium text-emerald-700 dark:text-emerald-300"><CheckCircle className="w-5 h-5 shrink-0 text-emerald-600 dark:text-emerald-400" /> Product category updated successfully.</div>}
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Edit product category</h1>
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa]">ID #{doc.id} • {doc.name} • /{doc.slug}</p>
        </div>
      </div>
      <ProductCategoryForm initial={doc} onSuccess={handleSaveSuccess} onCancel={handleBack} />
    </div>
  )
}
...
export default function EditProductCategoryPage(){
  return (
    <ClientOnly fallback={<EditProductCategorySkeleton />}>
      <EditProductCategoryContent />
    </ClientOnly>
  )
}
