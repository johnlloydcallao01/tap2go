...
import { useCatalogAttributes, type AttributeDoc, type CatalogAttributesResponse } from '@/hooks/useCatalogAttributes'
...
function AttributesPageContent(){
  ...
  const clearAll = () => { setQ(''); setDebouncedQ(''); setTypeFilter([]); setIsActiveFilter(null) }
  ...
  const handleDelete = async (force = false) => {
    ...
    try {
      ...
      const res = await fetch(`/api/catalog/attributes/${deleting.id}${qs}`, { method: 'DELETE' })
      const j = await res.json().catch(() => ({}))
      if (!res.ok) {
        if (j.code === 'HAS_TERMS' && !force) {
          setDeleteError(j.error || 'Attribute has terms')
          return
        }
        throw new Error(j.error || 'Failed to delete')
      }
      const deletedId = deleting.id
      // Optimistic removal across every cached attributes list (all pages/filters).
      ...
      queryClient.setQueriesData<CatalogAttributesResponse>(
        {
          queryKey: QUERY_KEYS.adminCatalogAttributes().slice(0, 3),
          predicate: (q) => q.queryKey.length === 4 && q.queryKey[2] === 'attributes' && typeof q.queryKey[3] === 'string',
        },
        (old) => {
      ...
      setDeleting(null)
      setDeleteError(null)
      // Bust every cached attributes list (all pages/filters) so the removal
    ...
    <div className="space-y-6 py-5 px-2.5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
            <span className="h-10 w-10 rounded-xl bg-[#eba236] text-white flex items-center justify-center"><Building className="w-5 h-5" /></span>
            Attributes
          </h1>
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1">Manage product attributes — color, size, and custom selectors for catalog variations.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link href="/catalog/attributes/new" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#eba236] hover:bg-[#c88a20] text-white rounded-xl text-sm font-semibold shadow-sm transition">
      ...
      {stats ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <KpiCard title="Total Attributes" value={String(stats.filteredTotal)} sub={`${stats.totalAll} overall`} icon={<Tag className="w-5 h-5 text-white" />} iconBg="bg-[#eba236]" />
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] p-3 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name, slug…" className="w-full pl-9 pr-9 py-2.5 text-sm bg-gray-50 dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#262626] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#eba236]/20 focus:border-[#eba236] text-gray-900 dark:text-white placeholder:text-gray-400" />
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#262626] space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <FilterPills label="Type" options={TYPE_OPTS} value={typeFilter} onToggle={toggleType} />
              <div>
        ...
        {activeFilterCount > 0 && !showFilters && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            {typeFilter.map((v) => <span key={v} className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-[#262626] text-gray-700 dark:text-[#a1a1aa] rounded-full text-xs font-medium">type:{v} <button onClick={() => toggleType(v)}><X className="w-3 h-3" /></button></span>)}
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] shadow-sm overflow-hidden">
        ...
        {isInitialLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <h3 className="font-semibold text-gray-900 dark:text-white">No attributes found</h3>
            <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1 max-w-md">Try adjusting search or filters, or create your first product attribute.</p>
            <Link href="/catalog/attributes/new" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-semibold"><Plus className="w-4 h-4" /> Create attribute</Link>
          </div>
        ) : !error && (
          <>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3 min-w-[160px]">
                          ...
                          <div className="min-w-0">
                            <div className="font-semibold text-gray-900 dark:text-white truncate max-w-[180px]">{a.name}</div>
                            <div className="text-xs text-gray-500 dark:text-[#a1a1aa] font-mono truncate max-w-[180px] sm:hidden">{a.slug}</div>
                            <div className="text-[11px] text-gray-400 flex items-center gap-1 mt-0.5"><CalendarDays className="w-3 h-3" /> {fmtDate(a.createdAt)}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 hidden sm:table-cell">
                        <span className="font-mono text-xs text-gray-700 dark:text-[#a1a1aa]">{a.slug}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium border capitalize ${typeBadge(a.type)}`}>{a.type}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border ${a.is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-300' : 'bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400'}`}>
                          <span className={`h-2 w-2 rounded-full ${a.is_active ? 'bg-emerald-500' : 'bg-zinc-400'}`} /> {a.is_active ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="px-4 py-3 hidden md:table-cell text-xs text-gray-500">{fmtDate(a.createdAt)}</td>
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex items-center gap-1">
                          <Link href={`/catalog/attributes/${a.id}`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-[#262626] text-gray-500 dark:text-[#a1a1aa] hover:text-gray-900 dark:hover:text-white" title="View"><Eye className="w-4 h-4" /></Link>
                          <Link href={`/catalog/attributes/${a.id}/edit`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-gray-500 dark:text-[#a1a1aa] hover:text-blue-600 dark:hover:text-blue-400" title="Edit"><Pencil className="w-4 h-4" /></Link>
                          <button onClick={() => { setDeleting(a); setDeleteError(null) }} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-500 dark:text-[#a1a1aa] hover:text-red-600 dark:hover:text-red-400" title="Delete"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {pagination && pagination.totalDocs > 0 && !loading && (
              <div className="px-4 py-3 border-t border-gray-200 dark:border-[#262626] flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
                <div className="text-gray-600 dark:text-[#a1a1aa]">Page {pagination.page} of {pagination.totalPages} • {pagination.totalDocs} attributes • 10 per page</div>
        ...
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => { if (!isDeleting) { setDeleting(null); setDeleteError(null) } }}>
            ...
            >
              ...
              <h3 className="font-bold text-gray-900 dark:text-white">Delete attribute?</h3>
              <p className="text-sm text-gray-600 dark:text-[#a1a1aa] mt-1">This will permanently delete <span className="font-semibold text-gray-900 dark:text-white">{deleting.name}</span> ({deleting.slug}). This action cannot be undone.</p>
              {deleteError && <div className="mt-3 p-2.5 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-xs text-amber-700 dark:text-amber-300">{deleteError} {deleteError.includes('HAS_TERMS') || deleteError.toLowerCase().includes('has terms') ? ' Use force delete to proceed.' : ''}</div>}
...
export default function AttributesPage(){
  return (
    <ClientOnly fallback={<AttributesSkeleton />}>
