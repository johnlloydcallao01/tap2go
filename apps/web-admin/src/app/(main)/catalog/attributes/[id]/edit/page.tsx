...
function EditAttributeSkeleton(){
...
function EditAttributeContent() {
  ...
  if (error || !doc) {
    return (
      <div className="space-y-6 py-5 px-2.5">
        <Link href="/catalog/attributes" className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-[#a1a1aa] hover:text-gray-900"><ArrowLeft className="w-4 h-4" /> Back to attributes</Link>
        <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626]">
          ...
          <Link href="/catalog/attributes" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-medium">Back</Link>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 py-5 px-2.5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Edit attribute</h1>
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa]">ID #{doc.id} • {doc.name}</p>
        </div>
      </div>
      <AttributeForm initial={doc} onSuccess={handleSaveSuccess} onCancel={handleBack} />
    </div>
  )
}
...
export default function EditAttributePage(){
  return (
    <ClientOnly fallback={<EditAttributeSkeleton />}>
