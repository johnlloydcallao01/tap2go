...
function EditVariationModifierGroupOverrideSkeleton(){
...
function EditVariationModifierGroupOverrideContent() {
  ...
  const handleSaveSuccess = async () => {
    setSaveSuccess(true)
    // Keep the list fresh for when the user navigates back to /catalog/variation-modifier-group-overrides.
    await queryClient.invalidateQueries({ queryKey: ['admin', 'catalog', 'variation-modifier-group-overrides'] })
    try {
      const res = await fetch(`/api/catalog/variation-modifier-group-overrides/${id}`, { cache: 'no-store' })
      const j = await res.json()
  ...
  if (error || !doc) {
    return (
      <div className="space-y-6 py-5 px-2.5">
        <Link href="/catalog/variation-modifier-group-overrides" className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-[#a1a1aa] hover:text-gray-900"><ArrowLeft className="w-4 h-4" /> Back to overrides</Link>
        <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626]">
          ...
          <Link href="/catalog/variation-modifier-group-overrides" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-medium">Back</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6 py-5 px-2.5">
      ...
      {saveSuccess && (
        <div className="flex items-center gap-2 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-sm font-medium text-emerald-700 dark:text-emerald-300">
          ...
          Variation modifier group override updated successfully.
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Edit variation modifier group override</h1>
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa]">ID #{doc.id} • {doc.mode}</p>
        </div>
      </div>
      <VariationModifierGroupOverrideForm initial={doc} onSuccess={handleSaveSuccess} onCancel={handleBack} />
    </div>
  )
}

export default function EditVariationModifierGroupOverridePage(){
  return (
    <ClientOnly fallback={<EditVariationModifierGroupOverrideSkeleton />}>
      <EditVariationModifierGroupOverrideContent />
    </ClientOnly>
  )
}
