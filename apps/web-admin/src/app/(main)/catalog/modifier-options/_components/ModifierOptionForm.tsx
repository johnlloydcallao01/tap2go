...
export function ModifierOptionForm({ initial, onSuccess, onCancel }: { initial?: ModifierOptionDoc | null; onSuccess: () => void; onCancel: () => void }) {
  const isEdit = !!initial
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const initialGroupId = initial?.modifier_group_id != null ? (typeof initial.modifier_group_id === 'number' ? String(initial.modifier_group_id) : String((initial.modifier_group_id as any).id)) : ''

  const [form, setForm] = useState({
    modifier_group_id: initialGroupId,
    name: initial?.name || '',
  ...
  useEffect(() => {
    if (!initial) return
    const gid = initial?.modifier_group_id != null ? (typeof initial.modifier_group_id === 'number' ? String(initial.modifier_group_id) : String((initial.modifier_group_id as any).id)) : ''
    setForm({
  ...
  const submit = async () => {
    setError(null)
    if (!form.modifier_group_id.trim()) return setError('modifier_group_id is required (numeric group id)')
    if (Number.isNaN(Number(form.modifier_group_id.trim()))) return setError('modifier_group_id must be numeric')
    ...
    try {
      const payload: any = {
        modifier_group_id: Number(form.modifier_group_id.trim()),
        name: form.name.trim(),
      ...
      const url = isEdit ? `/api/catalog/modifier-options/${(initial as any).id}` : '/api/catalog/modifier-options'
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div><label className={labelCls}>Modifier Group ID *</label><input value={form.modifier_group_id} onChange={(e)=>set('modifier_group_id', e.target.value)} placeholder="e.g. 5" className={inputCls} /></div>
