...
import { useModifierOptions, type ModifierOptionDoc, type ModifierOptionsResponse } from '@/hooks/useModifierOptions'
...
function groupLabel(v: ModifierOptionDoc['modifier_group_id']) {
  if (!v) return '—'
...
function ModifierOptionsSkeleton(){
  return (
...
function ModifierOptionsPageContent(){
  ...
  const [deleting, setDeleting] = useState<ModifierOptionDoc | null>(null)
  ...
  const qs = useMemo(() => {
    ...
    if (groupFilter.trim()) p.set('modifier_group_id', groupFilter.trim())
  ...
  const { data, isPending, isFetching, isError, error: queryError, refetch } = useModifierOptions(qs)
  ...
  const error = isError && !data && !hardRefreshing ? (queryError instanceof Error ? queryError.message : 'Failed to load modifier options') : null
  ...
  const handleHardRefresh = () => {
    ...
    void (async () => {
      try {
        queryClient.removeQueries({ queryKey: QUERY_KEYS.adminCatalogModifierOptions(qs) })
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const res = await fetch(`/api/catalog/modifier-options/${deleting.id}`, { method: 'DELETE' })
      ...
      queryClient.setQueriesData<ModifierOptionsResponse>(
        {
          queryKey: ['admin', 'catalog', 'modifier-options'],
          predicate: (q) => q.queryKey.length === 4 && q.queryKey[2] === 'modifier-options' && typeof q.queryKey[3] === 'string',
        },
        (old) => {
    ...
    <div className="space-y-6 py-5 px-2.5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1">Manage modifier options — price adjustments, defaults, and availability per group.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link href="/catalog/modifier-options/new" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#eba236] hover:bg-[#c88a20] text-white rounded-xl text-sm font-semibold shadow-sm transition">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input value={groupFilter} onChange={(e) => setGroupFilter(e.target.value)} placeholder="Group ID e.g. 5" className="w-full pl-9 pr-3 py-2.5 text-sm bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#262626] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#eba236]/20 focus:border-[#eba236] text-gray-900 dark:text-white" />
                </div>
              </div>
              <div>
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] shadow-sm overflow-hidden">
        ...
        {isInitialLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1 max-w-md">Try adjusting search or filters, or create your first modifier option.</p>
            <Link href="/catalog/modifier-options/new" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-semibold"><Plus className="w-4 h-4" /> Create option</Link>
          </div>
        ) : !error && (
          <>
                  ...
                  {docs.map((o) => (
                    <tr key={o.id} className="hover:bg-gray-50 dark:hover:bg-[#0a0a0a]/50 transition">
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell">
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#262626] rounded-full text-xs font-medium text-gray-700 dark:text-white"><Layers className="w-3 h-3 text-[#eba236]" /> {groupLabel(o.modifier_group_id)}</span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={async () => {
                          ...
                          const res = await fetch(`/api/catalog/modifier-options/${o.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_available: next }) })
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex items-center gap-1">
                          <Link href={`/catalog/modifier-options/${o.id}`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-[#262626] text-gray-500 dark:text-[#a1a1aa] hover:text-gray-900 dark:hover:text-white" title="View"><Eye className="w-4 h-4" /></Link>
                          <Link href={`/catalog/modifier-options/${o.id}/edit`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-gray-500 dark:text-[#a1a1aa] hover:text-blue-600 dark:hover:text-blue-400" title="Edit"><Pencil className="w-4 h-4" /></Link>
...
export default function ModifierOptionsPage(){
  return (
    <ClientOnly fallback={<ModifierOptionsSkeleton />}>
      <ModifierOptionsPageContent />
    </ClientOnly>
  )
}
