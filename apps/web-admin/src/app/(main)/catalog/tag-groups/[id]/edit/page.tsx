...
function EditTagGroupContent() {
  ...
  const handleSaveSuccess = async () => {
    setSaveSuccess(true)
    // Keep the list fresh for when the user navigates back to /catalog/tag-groups.
  ...
  if (error || !doc) {
    return (
      <div className="space-y-6 py-5 px-2.5">
        <Link href="/catalog/tag-groups" className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-[#a1a1aa] hover:text-gray-900"><ArrowLeft className="w-4 h-4" /> Back to tag groups</Link>
        <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626]">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white">Failed to load tag group</h3>
    ...
    <div className="space-y-6 py-5 px-2.5">
      ...
      {saveSuccess && (
        <div className="flex items-center gap-2 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-sm font-medium text-emerald-700 dark:text-emerald-300">
          ...
          Tag group updated successfully.
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Edit tag group</h1>
