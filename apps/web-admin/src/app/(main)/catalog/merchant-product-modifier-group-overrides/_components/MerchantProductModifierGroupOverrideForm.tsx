...
export function MerchantProductModifierGroupOverrideForm({ initial, onSuccess, onCancel }: { initial?: Doc | null; onSuccess: () => void; onCancel: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const url = isEdit ? `/api/catalog/merchant-product-modifier-group-overrides/${(initial as any).id}` : '/api/catalog/merchant-product-modifier-group-overrides'
  ...
  return (
    <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] shadow-sm overflow-hidden">
      <div className="p-6 space-y-6">
        ...
        <div>
          <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2"><Building className="w-4 h-4 text-[#eba236]" /> Merchant Product & Base Group</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              {form.merchant_product_id && baseGroupChoices.length === 0 && !loadingGroups && <p className="text-xs text-gray-400 mt-1">Only product-level modifier groups from the selected merchant product catalog item are allowed.</p>}
            </div>
            <div>
