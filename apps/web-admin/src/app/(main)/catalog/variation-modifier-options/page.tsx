...
import { useVariationModifierOptions, type VariationModifierOptionDoc, type VariationModifierOptionsResponse } from '@/hooks/useVariationModifierOptions'
import { useVariationModifierGroups } from '@/hooks/useVariationModifierGroups'
import { ClientOnly } from '@/components/ClientOnly'
import {
  Building, Search, X, SlidersHorizontal, ChevronDown, Plus, RefreshCw, AlertCircle,
  CheckCircle, XCircle, Eye, Pencil, Trash2, CalendarDays, Tag, Layers, Coins, ToggleLeft, Star
} from '@/components/ui/IconWrapper'

function groupLabel(v: VariationModifierOptionDoc['variation_modifier_group_id']) {
  if (!v) return '—'
...
function VariationModifierOptionsSkeleton(){
  return (
...
function VariationModifierOptionsPageContent(){
  const [q, setQ] = useState('')
  const [debouncedQ, setDebouncedQ] = useState('')
  const [groupFilter, setGroupFilter] = useState('')
  const [isAvailableFilter, setIsAvailableFilter] = useState<boolean | null>(null)
  const [sort, setSort] = useState<string>('-createdAt')
  const [page, setPage] = useState(1)
  const limit = 10
  const [showFilters, setShowFilters] = useState(false)

  const [deleting, setDeleting] = useState<VariationModifierOptionDoc | null>(null)
  const [isDeleting, setIsDeleting] = useState(false)

  useEffect(() => { const id = setTimeout(() => setDebouncedQ(q.trim()), 400); return () => clearTimeout(id) }, [q])

  // modifier group dropdown — cached via the shared variation-modifier-groups query
  const { data: groupsData } = useVariationModifierGroups('limit=100')
  const groupChoices = (groupsData?.docs || []).map((d) => ({ id: d.id, name: d.name || `#${d.id}` }))

  const activeFilterCount = useMemo(() => {
    return (groupFilter.trim() ? 1 : 0) + (isAvailableFilter !== null ? 1 : 0) + (debouncedQ ? 1 : 0)
  }, [groupFilter, isAvailableFilter, debouncedQ])
  ...
  const qs = useMemo(() => {
    ...
    if (groupFilter.trim()) p.set('variation_modifier_group_id', groupFilter.trim())
    if (isAvailableFilter !== null) p.set('is_available', String(isAvailableFilter))
    return p.toString()
  }, [page, limit, sort, debouncedQ, groupFilter, isAvailableFilter])

  const queryClient = useQueryClient()
  const [hardRefreshing, setHardRefreshing] = useState(false)
  const { data, isPending, isFetching, isError, error: queryError, refetch } = useVariationModifierOptions(qs)
  ...
  const error = isError && !data && !hardRefreshing ? (queryError instanceof Error ? queryError.message : 'Failed to load variation modifier options') : null
  ...
  const handleHardRefresh = () => {
    ...
    void (async () => {
      try {
        queryClient.removeQueries({ queryKey: QUERY_KEYS.adminCatalogVariationModifierOptions(qs) })
        await refetch({ cancelRefetch: true })
      } finally { setHardRefreshing(false) }
    })()
  }

  useEffect(() => { setPage(1) }, [debouncedQ, groupFilter, isAvailableFilter, sort])
  ...
  const clearAll = () => { setQ(''); setDebouncedQ(''); setGroupFilter(''); setIsAvailableFilter(null) }

  const handleDelete = async () => {
    if (!deleting || isDeleting) return
    setIsDeleting(true)
    try {
      const res = await fetch(`/api/catalog/variation-modifier-options/${deleting.id}`, { method: 'DELETE' })
      const j = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(j.error || 'Failed to delete')
      const deletedId = deleting.id
      // Optimistic removal across every cached options list (all pages/filters).
      ...
      queryClient.setQueriesData<VariationModifierOptionsResponse>(
        {
          queryKey: ['admin', 'catalog', 'variation-modifier-options'],
          predicate: (q) => q.queryKey.length === 4 && q.queryKey[2] === 'variation-modifier-options' && typeof q.queryKey[3] === 'string',
        },
        (old) => {
      ...
      setDeleting(null)
      // Bust every cached options list (all pages/filters) so the removal
    ...
    <div className="space-y-6 py-5 px-2.5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1">Manage variation modifier options — price adjustments, defaults, and availability per variation group.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={handleHardRefresh}
            disabled={loading}
            aria-label="Refresh variation modifier options"
          ...
          <Link href="/catalog/variation-modifier-options/new" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#eba236] hover:bg-[#c88a20] text-white rounded-xl text-sm font-semibold shadow-sm transition">
      ...
      {stats ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <KpiCard title="Total Options" value={String(stats.filteredTotal)} sub={`${stats.totalAll} overall`} icon={<Coins className="w-5 h-5 text-white" />} iconBg="bg-[#eba236]" />
          <KpiCard title="Available" value={String(stats.availableCount)} sub={`${stats.unavailableCount} unavailable`} icon={<CheckCircle className="w-5 h-5 text-white" />} iconBg="bg-emerald-500" />
          <KpiCard title="Unavailable" value={String(stats.unavailableCount)} sub="hidden" icon={<XCircle className="w-5 h-5 text-white" />} iconBg="bg-zinc-500" />
          <KpiCard title="Default" value={String(stats.defaultCount)} sub="is_default" icon={<Star className="w-5 h-5 text-white" />} iconBg="bg-blue-600" />
        </div>
      ) : isInitialLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-pulse">
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] p-3 shadow-sm">
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#262626] space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <p className="text-xs font-semibold text-gray-700 dark:text-[#a1a1aa] mb-2">Variation Modifier Group</p>
                <select value={groupFilter} onChange={(e) => setGroupFilter(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#0a0a0a] text-sm text-gray-700 dark:text-white">
              ...
              <div>
                <p className="text-xs font-semibold text-gray-700 dark:text-[#a1a1aa] mb-2">Availability</p>
                <div className="flex flex-wrap gap-1.5">
                  {[['all','All'],['true','Available only'],['false','Unavailable only']].map(([v,l]) => {
                    const active = (isAvailableFilter===null && v==='all') || String(isAvailableFilter)===v
                    return <button key={v} onClick={() => setIsAvailableFilter(v==='all'?null: v==='true')} className={`px-2.5 py-1 rounded-full text-xs font-medium border ${active ? 'bg-[#eba236] text-white border-[#eba236]' : 'bg-white dark:bg-[#0a0a0a] text-gray-700 dark:text-[#a1a1aa] border-gray-200 dark:border-[#262626]'}`}>{l}</button>
                  })}
                </div>
              </div>
            </div>
        ...
        {activeFilterCount > 0 && !showFilters && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            {groupFilter.trim() && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-[#262626] text-gray-700 dark:text-[#a1a1aa] rounded-full text-xs font-medium">group:{groupFilter} <button onClick={() => setGroupFilter('')}><X className="w-3 h-3" /></button></span>}
            {isAvailableFilter !== null && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-[#262626] rounded-full text-xs">{isAvailableFilter ? 'Available only' : 'Unavailable only'} <button onClick={() => setIsAvailableFilter(null)}><X className="w-3 h-3" /></button></span>}
          </div>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] shadow-sm overflow-hidden">
        {error && (
          <div className="flex flex-col items-center justify-center py-16 px-6">
            ...
            <h3 className="font-semibold text-gray-900 dark:text-white">Failed to load variation modifier options</h3>
        ...
        {isInitialLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <h3 className="font-semibold text-gray-900 dark:text-white">No variation modifier options found</h3>
            <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1 max-w-md">Try adjusting search or filters, or create your first variation modifier option.</p>
            <Link href="/catalog/variation-modifier-options/new" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-semibold"><Plus className="w-4 h-4" /> Create option</Link>
          </div>
        ) : !error && (
          <>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-[#0a0a0a] text-xs text-gray-500 dark:text-[#a1a1aa] border-b border-gray-200 dark:border-[#262626]">
                  <tr>
                    ...
                    <th className="text-right px-4 py-3 font-medium">Price Adj.</th>
                  ...
                  {docs.map((o) => (
                    <tr key={o.id} className="hover:bg-gray-50 dark:hover:bg-[#0a0a0a]/50 transition">
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell">
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#262626] rounded-full text-xs font-medium text-gray-700 dark:text-white"><Layers className="w-3 h-3 text-[#eba236]" /> {groupLabel(o.variation_modifier_group_id)}</span>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <span className="font-mono text-xs font-semibold text-gray-900 dark:text-white">{fmtPrice(o.price_adjustment)}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium border ${o.is_default ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:text-amber-300' : 'bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400'}`}>{o.is_default ? 'Default' : '—'}</span>
                      </td>
                      <td className="px-4 py-3">
                        <button onClick={async () => {
                          const next = !o.is_available
                          const res = await fetch(`/api/catalog/variation-modifier-options/${o.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_available: next }) })
                          if (!res.ok) { const j = await res.json().catch(()=>({})); alert(j.error || 'Failed to toggle') } else { void refetch() }
                        }} className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border transition ${o.is_available ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-300' : 'bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400'}`}>
                          <span className={`h-2 w-2 rounded-full ${o.is_available ? 'bg-emerald-500' : 'bg-zinc-400'}`} /> {o.is_available ? 'Available' : 'Unavailable'}
                        </button>
                      </td>
                      <td className="px-4 py-3 hidden xl:table-cell text-xs text-gray-500">{o.sort_order}</td>
                      <td className="px-4 py-3 hidden md:table-cell text-xs text-gray-500">{fmtDate(o.createdAt)}</td>
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex items-center gap-1">
                          <Link href={`/catalog/variation-modifier-options/${o.id}`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-[#262626] text-gray-500 dark:text-[#a1a1aa] hover:text-gray-900 dark:hover:text-white" title="View"><Eye className="w-4 h-4" /></Link>
                          <Link href={`/catalog/variation-modifier-options/${o.id}/edit`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-gray-500 dark:text-[#a1a1aa] hover:text-blue-600 dark:hover:text-blue-400" title="Edit"><Pencil className="w-4 h-4" /></Link>
            ...
            {pagination && pagination.totalDocs > 0 && !loading && (
              <div className="px-4 py-3 border-t border-gray-200 dark:border-[#262626] flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
                <div className="text-gray-600 dark:text-[#a1a1aa]">Page {pagination.page} of {pagination.totalPages} • {pagination.totalDocs} options • 10 per page</div>
        ...
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => { if (!isDeleting) setDeleting(null) }}>
            ...
            >
              ...
              <h3 className="font-bold text-gray-900 dark:text-white">Delete modifier option?</h3>
              <p className="text-sm text-gray-600 dark:text-[#a1a1aa] mt-1">This will permanently delete <span className="font-semibold text-gray-900 dark:text-white">{deleting.name}</span> (#{deleting.id}). This action cannot be undone.</p>
...
export default function VariationModifierOptionsPage(){
  return (
    <ClientOnly fallback={<VariationModifierOptionsSkeleton />}>
      <VariationModifierOptionsPageContent />
    </ClientOnly>
  )
}
