...
type Stats = { total: number; totalAll: number; filteredTotal: number; modeBreakdown: Record<string, number>; sourceBreakdown: Record<string, number>; activeCount: number; inactiveCount: number }
...
function MerchantVariationModifierOptionOverridesPageContent(){
  ...
  const load = useCallback(async (opts?: { hard?: boolean }) => {
    ...
    try {
      ...
      const res = await fetch(`/api/catalog/merchant-variation-modifier-option-overrides?${bust}`, { cache: 'no-store' })
      if (!res.ok) {
