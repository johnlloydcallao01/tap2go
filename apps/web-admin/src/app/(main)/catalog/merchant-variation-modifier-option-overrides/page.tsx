...
import { useMerchantVariationModifierOptionOverrides, type OverrideDoc, type MerchantVariationModifierOptionOverridesResponse } from '@/hooks/useMerchantVariationModifierOptionOverrides'
import {
...
const TARGET_OPTS: { value: string; label: string }[] = [
...
function variationLabel(v: OverrideDoc['variation_id']) {
  if (!v) return '—'
  if (typeof v === 'number') return `#${v}`
  return (v as any).name ? `${(v as any).name} (#${(v as any).id})` : (v as any).sku ? `${(v as any).sku} (#${(v as any).id})` : `#${(v as any).id}`
}
function optionLabel(g: any) {
  if (!g) return '—'
...
function MerchantVariationModifierOptionOverridesSkeleton(){
  return (
...
function MerchantVariationModifierOptionOverridesPageContent(){
  ...
  const [targetSourceFilter, setTargetSourceFilter] = useState<string[]>([])
  ...
  const [variationOptionFilter, setVariationOptionFilter] = useState('')
  ...
  useEffect(() => {
    fetch('/api/merchant-products?limit=100', { cache: 'no-store' })
      .then((r) => r.json())
      .then((j) => {
        const docsArr: any[] = j.docs || j.data?.docs || []
        setMerchantProductChoices(docsArr.map((d: any) => ({ id: d.id, display_title: d.display_title || d.displayTitle || `#${d.id}`, productId: d.product_id != null ? (typeof d.product_id === 'number' ? d.product_id : Number(d.product_id?.id ?? null)) : (d.product != null ? (typeof d.product === 'number' ? d.product : Number(d.product?.id ?? null)) : null) })))
      })
      .catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    if (!mpId) {
      fetch('/api/catalog/variations?limit=50', { cache: 'no-store' })
        .then((r) => r.json())
        .then((j) => {
          const arr: any[] = j.docs || []
          setVariationChoices(arr.map((d: any) => ({ id: d.id, name: d.name || d.sku || `#${d.id}` })))
        })
    ...
    const doFetch = async (productId: number) => {
      try {
        ...
        const filtered = arr.filter((d: any) => {
          ...
          return nid === productId
        })
        if (filtered.length) setVariationChoices(filtered.map((d: any) => ({ id: d.id, name: d.name || d.sku || `#${d.id}` })))
        else {
          const rr = await fetch(`/api/catalog/variations?productId=${productId}&limit=100`, { cache: 'no-store' })
          const jj = await rr.json()
          const a: any[] = jj.docs || []
          setVariationChoices(a.map((d: any) => ({ id: d.id, name: d.name || d.sku || `#${d.id}` })))
        }
      } catch { setVariationChoices([]) }
    }
  ...
  useEffect(() => {
    ...
    const doFetch = async (productId: number) => {
      try {
        ...
        setBaseOptionChoices(filtered.map((d: any) => ({ id: d.id, name: d.name || `#${d.id}` })))
      } catch { setBaseOptionChoices([]) }
    }
    if (pid != null && Number.isFinite(pid)) void doFetch(pid)
    else {
  ...
  useEffect(() => {
    ...
    if (!vid) {
      fetch('/api/catalog/variation-modifier-options?limit=50', { cache: 'no-store' })
        .then((r) => r.json())
        .then((j) => {
          const arr: any[] = j.docs || []
          setVariationOptionChoices(arr.map((d: any) => ({ id: d.id, name: d.name || `#${d.id}` })))
        })
    ...
    fetch(`/api/catalog/variation-modifier-groups?variationId=${vid}&limit=100`, { cache: 'no-store' })
      ...
      .then(async (j) => {
        ...
        setVariationOptionChoices(filtered.map((d: any) => ({ id: d.id, name: d.name || `#${d.id}` })))
      })
      .catch(() => setVariationOptionChoices([]))
  }, [variationFilter])

  const activeFilterCount = useMemo(() => {
    return modeFilter.length + targetSourceFilter.length + (isActiveFilter !== null ? 1 : 0) + (merchantProductFilter.trim() ? 1 : 0) + (variationFilter.trim() ? 1 : 0) + (baseOptionFilter.trim() ? 1 : 0) + (variationOptionFilter.trim() ? 1 : 0) + (debouncedQ ? 1 : 0)
  }, [modeFilter, targetSourceFilter, isActiveFilter, merchantProductFilter, variationFilter, baseOptionFilter, variationOptionFilter, debouncedQ])
  ...
  const qs = useMemo(() => {
    ...
    if (targetSourceFilter.length) p.set('target_option_source', targetSourceFilter.join(','))
    ...
    if (variationOptionFilter.trim()) p.set('variation_modifier_option_id', variationOptionFilter.trim())
    return p.toString()
  }, [page, limit, sort, debouncedQ, modeFilter, targetSourceFilter, isActiveFilter, merchantProductFilter, variationFilter, baseOptionFilter, variationOptionFilter])

  const queryClient = useQueryClient()
  const [hardRefreshing, setHardRefreshing] = useState(false)
  const { data, isPending, isFetching, isError, error: queryError, refetch } = useMerchantVariationModifierOptionOverrides(qs)
  ...
  const handleHardRefresh = () => {
    ...
    void (async () => {
      try {
        queryClient.removeQueries({ queryKey: QUERY_KEYS.adminCatalogMerchantVariationModifierOptionOverrides(qs) })
        await refetch({ cancelRefetch: true })
      } finally { setHardRefreshing(false) }
    })()
  }

  useEffect(() => { setPage(1) }, [debouncedQ, modeFilter, targetSourceFilter, isActiveFilter, merchantProductFilter, variationFilter, baseOptionFilter, variationOptionFilter, sort])
  ...
  const toggleTarget = (v: string) => setTargetSourceFilter((prev) => prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v])
  const clearAll = () => { setQ(''); setDebouncedQ(''); setModeFilter([]); setTargetSourceFilter([]); setIsActiveFilter(null); setMerchantProductFilter(''); setVariationFilter(''); setBaseOptionFilter(''); setVariationOptionFilter('') }

  const handleDelete = async () => {
    if (!deleting || isDeleting) return
    setIsDeleting(true)
    try {
      const res = await fetch(`/api/catalog/merchant-variation-modifier-option-overrides/${deleting.id}`, { method: 'DELETE' })
      const j = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(j.error || 'Failed to delete')
      const deletedId = deleting.id
      // Optimistic removal across every cached overrides list (all pages/filters).
      ...
      queryClient.setQueriesData<MerchantVariationModifierOptionOverridesResponse>(
        {
          queryKey: ['admin', 'catalog', 'merchant-variation-modifier-option-overrides'],
          predicate: (q) => q.queryKey.length === 4 && q.queryKey[2] === 'merchant-variation-modifier-option-overrides' && typeof q.queryKey[3] === 'string',
        },
        (old) => {
      ...
      setDeleting(null)
      // Bust every cached overrides list (all pages/filters) so the removal
      // reconciles with the server instead of waiting out the 3-min staleTime.
      await queryClient.invalidateQueries({ queryKey: ['admin', 'catalog', 'merchant-variation-modifier-option-overrides'] })
      await refetch()
    } catch (e: any) { alert(e?.message || 'Delete failed') }
    finally { setIsDeleting(false) }
  }

  const showBasePicker = targetSourceFilter.length === 0 || targetSourceFilter.includes('product_base')
  const showVarPicker = targetSourceFilter.includes('variation_added')
    ...
    <div className="space-y-6 py-5 px-2.5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1">Merchant-level variation overrides for modifier options — inherit, hide or override per merchant product variation.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link href="/catalog/merchant-variation-modifier-option-overrides/new" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#eba236] hover:bg-[#c88a20] text-white rounded-xl text-sm font-semibold shadow-sm transition">
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] p-3 shadow-sm">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#262626] space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
              <div>
                ...
                <select value={merchantProductFilter} onChange={(e) => { setMerchantProductFilter(e.target.value); setVariationFilter(''); setBaseOptionFilter(''); setVariationOptionFilter('') }} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#0a0a0a] text-sm text-gray-700 dark:text-white">
              ...
              <div>
                ...
                <select value={variationFilter} onChange={(e) => { setVariationFilter(e.target.value); setVariationOptionFilter('') }} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#0a0a0a] text-sm text-gray-700 dark:text-white">
              ...
              {showBasePicker && (
                <div>
                  <p className="text-xs font-semibold text-gray-700 dark:text-[#a1a1aa] mb-2">Base Option (product_base)</p>
                  <select value={baseOptionFilter} onChange={(e) => setBaseOptionFilter(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#0a0a0a] text-sm text-gray-700 dark:text-white">
                    <option value="">All base options</option>
              ...
              {showVarPicker && (
                <div>
                  <p className="text-xs font-semibold text-gray-700 dark:text-[#a1a1aa] mb-2">Variation Option (variation_added)</p>
                  <select value={variationOptionFilter} onChange={(e) => setVariationOptionFilter(e.target.value)} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#0a0a0a] text-sm text-gray-700 dark:text-white">
                    <option value="">All variation options</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <FilterPills label="Target Source" options={TARGET_OPTS} value={targetSourceFilter} onToggle={toggleTarget} />
        ...
        {activeFilterCount > 0 && !showFilters && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            ...
            {targetSourceFilter.map((v) => <span key={v} className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-[#262626] text-gray-700 dark:text-[#a1a1aa] rounded-full text-xs font-medium">source:{v} <button onClick={() => toggleTarget(v)}><X className="w-3 h-3" /></button></span>)}
            {baseOptionFilter.trim() && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-[#262626] text-gray-700 dark:text-[#a1a1aa] rounded-full text-xs font-medium">base:{baseOptionFilter} <button onClick={() => setBaseOptionFilter('')}><X className="w-3 h-3" /></button></span>}
            {variationOptionFilter.trim() && <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-[#262626] text-gray-700 dark:text-[#a1a1aa] rounded-full text-xs font-medium">varOption:{variationOptionFilter} <button onClick={() => setVariationOptionFilter('')}><X className="w-3 h-3" /></button></span>}
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] shadow-sm overflow-hidden">
        ...
        {isInitialLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1 max-w-md">Try adjusting search or filters, or create your first merchant variation modifier option override.</p>
            <Link href="/catalog/merchant-variation-modifier-option-overrides/new" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-semibold"><Plus className="w-4 h-4" /> Create override</Link>
          </div>
        ) : !error && (
          <>
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 dark:bg-[#0a0a0a] text-xs text-gray-500 dark:text-[#a1a1aa] border-b border-gray-200 dark:border-[#262626]">
                  <tr>
                    ...
                    <th className="text-left px-4 py-3 font-medium">Base / Variation Option</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-[#262626]">
                  {docs.map((d) => (
                    <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-[#0a0a0a]/50 transition">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#262626] rounded-full text-xs font-medium text-gray-700 dark:text-white"><Layers className="w-3 h-3 text-[#eba236]" /> {variationLabel(d.variation_id)}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium border capitalize ${sourceBadge(d.target_option_source)}`}>{d.target_option_source.replace('_',' ')}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1 px-2 py-1 bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#262626] rounded-full text-xs font-medium text-gray-700 dark:text-white"><Layers className="w-3 h-3 text-[#eba236]" /> {d.target_option_source === 'product_base' ? optionLabel(d.base_modifier_option_id) : optionLabel(d.variation_modifier_option_id)}</span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={async () => {
                          ...
                          const res = await fetch(`/api/catalog/merchant-variation-modifier-option-overrides/${d.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_active: next }) })
                          if (!res.ok) { const j = await res.json().catch(()=>({})); alert(j.error || 'Failed to toggle') }
                          void refetch()
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex items-center gap-1">
                          <Link href={`/catalog/merchant-variation-modifier-option-overrides/${d.id}`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-[#262626] text-gray-500 dark:text-[#a1a1aa] hover:text-gray-900 dark:hover:text-white" title="View"><Eye className="w-4 h-4" /></Link>
                          <Link href={`/catalog/merchant-variation-modifier-option-overrides/${d.id}/edit`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-gray-500 dark:text-[#a1a1aa] hover:text-blue-600 dark:hover:text-blue-400" title="Edit"><Pencil className="w-4 h-4" /></Link>
...
export default function MerchantVariationModifierOptionOverridesPage(){
  return (
    <ClientOnly fallback={<MerchantVariationModifierOptionOverridesSkeleton />}>
      <MerchantVariationModifierOptionOverridesPageContent />
    </ClientOnly>
  )
}
