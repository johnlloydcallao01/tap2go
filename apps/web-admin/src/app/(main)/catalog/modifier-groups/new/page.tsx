...
function NewModifierGroupSkeleton(){
...
function NewModifierGroupContent() {
  ...
  const handleSuccess = async () => {
    // Bust the 3-min list cache BEFORE navigating back, otherwise /catalog/modifier-groups
  ...
  return (
    <div className="space-y-6 py-5 px-2.5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">New modifier group</h1>
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa]">Create a modifier group for a product.</p>
        </div>
      </div>
      <ModifierGroupForm onSuccess={handleSuccess} onCancel={handleBack} />
    </div>
  )
}
...
export default function NewModifierGroupPage(){
  return (
    <ClientOnly fallback={<NewModifierGroupSkeleton />}>
      <NewModifierGroupContent />
    </ClientOnly>
  )
}
