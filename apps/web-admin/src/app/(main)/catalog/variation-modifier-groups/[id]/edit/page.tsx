...
function EditVariationModifierGroupSkeleton(){
...
function EditVariationModifierGroupContent() {
  ...
  const handleSaveSuccess = async () => {
    setSaveSuccess(true)
    // Keep the list fresh for when the user navigates back to /catalog/variation-modifier-groups.
  ...
  if (error || !doc) {
    return (
      <div className="space-y-6 py-5 px-2.5">
        <Link href="/catalog/variation-modifier-groups" className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-[#a1a1aa] hover:text-gray-900"><ArrowLeft className="w-4 h-4" /> Back to variation modifier groups</Link>
        <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626]">
          ...
          <Link href="/catalog/variation-modifier-groups" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-medium">Back</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6 py-5 px-2.5">
      ...
      {saveSuccess && (
        <div className="flex items-center gap-2 p-4 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-sm font-medium text-emerald-700 dark:text-emerald-300">
          ...
          Variation modifier group updated successfully.
        </div>
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Edit variation modifier group</h1>
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa]">ID #{doc.id} • {doc.name}</p>
        </div>
      </div>
      <VariationModifierGroupForm initial={doc} onSuccess={handleSaveSuccess} onCancel={handleBack} />
    </div>
  )
}

export default function EditVariationModifierGroupPage(){
  return (
    <ClientOnly fallback={<EditVariationModifierGroupSkeleton />}>
      <EditVariationModifierGroupContent />
    </ClientOnly>
  )
}
