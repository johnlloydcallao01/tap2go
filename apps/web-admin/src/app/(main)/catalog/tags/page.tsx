...
export default function TagsPage() {
  ...
  const load = useCallback(async (opts?: { hard?: boolean }) => {
    ...
    try {
      ...
      const res = await fetch(`/api/catalog/tags?${bust}`, { cache: 'no-store' })
      if (!res.ok) {
        const text = await res.text()
        try { const j = JSON.parse(text); throw new Error(j.error || 'Failed to load tags') } catch { throw new Error(text || 'Failed to load tags') }
      }
      const json = await res.json()
      setDocs(json.docs || [])
      setPagination(json.pagination || null)
      setStats(json.stats || null)
    } catch (e: any) { setError(e?.message || 'Failed to load tags') } finally { setLoading(false) }
  }, [buildQuery])
