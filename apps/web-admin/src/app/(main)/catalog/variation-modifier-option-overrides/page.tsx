...
import { useVariationModifierOptionOverrides, type VariationModifierOptionOverrideDoc as OverrideDoc, type VariationModifierOptionOverridesResponse } from '@/hooks/useVariationModifierOptionOverrides'
...
function VariationModifierOptionOverridesPageContent(){
  ...
  // base modifier option dropdown — depends on the selected variation's product
  const selectedProductId = variationChoices.find((v) => String(v.id) === variationFilter.trim())?.productId ?? null
  const hasProduct = !!variationFilter.trim() && selectedProductId != null && Number.isFinite(selectedProductId)
  const groupsQs = useMemo(() => {
    return hasProduct ? `productId=${selectedProductId}&limit=100` : 'limit=50'
  }, [hasProduct, selectedProductId])
  const { data: groupsData } = useModifierGroups(groupsQs)
  const groupIds = useMemo(() => (groupsData?.docs || []).map((g) => Number(g.id)).filter((n) => Number.isFinite(n)), [groupsData])

  const optionsQs = hasProduct ? 'limit=500' : 'limit=50'
  const { data: optionsData } = useModifierOptions(optionsQs)
  const baseOptionChoices = useMemo(() => {
    const opts = optionsData?.docs || []
    if (hasProduct) {
      if (groupIds.length === 0) return []
      return opts
        .filter((o) => {
          const gid = o.modifier_group_id != null ? (typeof o.modifier_group_id === 'number' ? o.modifier_group_id : Number((o.modifier_group_id as any)?.id ?? NaN)) : NaN
          return Number.isFinite(gid) && groupIds.includes(gid)
        })
        .map((o) => ({ id: o.id, name: o.name || `#${o.id}` }))
    }
    return opts.map((o) => ({ id: o.id, name: o.name || `#${o.id}` }))
  }, [optionsData, hasProduct, groupIds])

  const activeFilterCount = useMemo(() => {
    return modeFilter.length + availabilityFilter.length + (isActiveFilter !== null ? 1 : 0) + (variationFilter.trim() ? 1 : 0) + (baseOptionFilter.trim() ? 1 : 0) + (debouncedQ ? 1 : 0)
  }, [modeFilter, availabilityFilter, isActiveFilter, variationFilter, baseOptionFilter, debouncedQ])
  ...
  const qs = useMemo(() => {
    ...
    return p.toString()
  }, [page, limit, sort, debouncedQ, modeFilter, availabilityFilter, isActiveFilter, variationFilter, baseOptionFilter])

  const queryClient = useQueryClient()
  const [hardRefreshing, setHardRefreshing] = useState(false)
  const { data, isPending, isFetching, isError, error: queryError, refetch } = useVariationModifierOptionOverrides(qs)
  ...
  const handleHardRefresh = () => {
    ...
    void (async () => {
      try {
        queryClient.removeQueries({ queryKey: QUERY_KEYS.adminCatalogVariationModifierOptionOverrides(qs) })
        await refetch({ cancelRefetch: true })
      } finally { setHardRefreshing(false) }
    })()
  }

  useEffect(() => { setPage(1) }, [debouncedQ, modeFilter, availabilityFilter, isActiveFilter, variationFilter, baseOptionFilter, sort])
  ...
  const clearAll = () => { setQ(''); setDebouncedQ(''); setModeFilter([]); setAvailabilityFilter([]); setIsActiveFilter(null); setVariationFilter(''); setBaseOptionFilter('') }

  const handleDelete = async () => {
    if (!deleting || isDeleting) return
    setIsDeleting(true)
    try {
      const res = await fetch(`/api/catalog/variation-modifier-option-overrides/${deleting.id}`, { method: 'DELETE' })
      ...
      queryClient.setQueriesData<VariationModifierOptionOverridesResponse>(
        {
          queryKey: ['admin', 'catalog', 'variation-modifier-option-overrides'],
          predicate: (q) => q.queryKey.length === 4 && q.queryKey[2] === 'variation-modifier-option-overrides' && typeof q.queryKey[3] === 'string',
        },
        (old) => {
    ...
    <div className="space-y-6 py-5 px-2.5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1">Hybrid rules for inherited product-level modifier options — pricing, availability per variation.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link href="/catalog/variation-modifier-option-overrides/new" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#eba236] hover:bg-[#c88a20] text-white rounded-xl text-sm font-semibold shadow-sm transition">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-[#262626] space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
              <div>
                ...
                <select value={variationFilter} onChange={(e) => { setVariationFilter(e.target.value); setBaseOptionFilter('') }} className="w-full px-3 py-2.5 rounded-lg border border-gray-200 dark:border-[#262626] bg-white dark:bg-[#0a0a0a] text-sm text-gray-700 dark:text-white">
      ...
      <div className="bg-white dark:bg-[#171717] rounded-xl border border-gray-200 dark:border-[#262626] shadow-sm overflow-hidden">
        ...
        {isInitialLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-[#a1a1aa] mt-1 max-w-md">Try adjusting search or filters, or create your first variation modifier option override.</p>
            <Link href="/catalog/variation-modifier-option-overrides/new" className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-[#eba236] text-white rounded-lg text-sm font-semibold"><Plus className="w-4 h-4" /> Create override</Link>
          </div>
        ) : !error && (
          <>
                    ...
                    <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-[#0a0a0a]/50 transition">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={async () => {
                          ...
                          const res = await fetch(`/api/catalog/variation-modifier-option-overrides/${d.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_active: next }) })
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="inline-flex items-center gap-1">
                          <Link href={`/catalog/variation-modifier-option-overrides/${d.id}`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-[#262626] text-gray-500 dark:text-[#a1a1aa] hover:text-gray-900 dark:hover:text-white" title="View"><Eye className="w-4 h-4" /></Link>
                          <Link href={`/catalog/variation-modifier-option-overrides/${d.id}/edit`} className="h-7 w-7 inline-flex items-center justify-center rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-gray-500 dark:text-[#a1a1aa] hover:text-blue-600 dark:hover:text-blue-400" title="Edit"><Pencil className="w-4 h-4" /></Link>
