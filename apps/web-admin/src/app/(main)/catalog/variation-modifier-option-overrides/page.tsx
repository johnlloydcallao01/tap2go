...
function VariationModifierOptionOverridesPageContent(){
  ...
  useEffect(() => {
    fetch('/api/catalog/variations?limit=100', { cache: 'no-store' })
      ...
      .then((j) => {
        ...
        setVariationChoices(arr.map((d: any) => ({ id: d.id, name: d.name || d.sku || `#${d.id}`, productId: d.product_id != null ? (typeof d.product_id === 'number' ? d.product_id : Number(d.product_id?.id ?? null)) : null })))
      })
      .catch(() => {})
  }, [])
  ...
  const load = useCallback(async (opts?: { hard?: boolean }) => {
    ...
    try {
      ...
      const res = await fetch(`/api/catalog/variation-modifier-option-overrides?${bust}`, { cache: 'no-store' })
      if (!res.ok) {
