...
function EditVariationSkeleton(){
...
function EditVariationContent() {
  ...
  if (error || !doc) {
    return (
      <div className="space-y-6 py-5 px-2.5">
        <Link href="/catalog/variations" className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-[#a1a1aa] hover:text-gray-900">
  ...
  return (
    <div className="space-y-6 py-5 px-2.5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Edit variation</h1>
          <p className="text-sm text-gray-500 dark:text-[#a1a1aa]">ID #{doc.id} • {doc.sku || doc.name || '—'}</p>
        </div>
      </div>
      <VariationForm initial={doc} onSuccess={handleSaveSuccess} onCancel={handleBack} />
    </div>
  )
}
...
export default function EditVariationPage(){
  return (
    <ClientOnly fallback={<EditVariationSkeleton />}>
