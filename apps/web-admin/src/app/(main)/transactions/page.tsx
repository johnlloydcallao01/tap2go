...
import { TransactionsPageContent, TransactionsSkeleton } from './content';

// Default list query — must match the qs TransactionsPageContent builds for
// page=1&limit=10&sort=-paid_at with no search/filters
// (URLSearchParams order: page, limit, sort).
const DEFAULT_QS = 'page=1&limit=10&sort=-paid_at';
...
export default async function TransactionsPage() {
  ...
  try {
    ...
    if (token) {
      ...
      const res = await fetch(`${base}/api/admin/transactions?${DEFAULT_QS}`, {
        headers: { Authorization: `JWT ${token}` },
        next: { revalidate: 30 },
      });
      if (res.ok) {
        queryClient.setQueryData(QUERY_KEYS.adminTransactions(DEFAULT_QS), await res.json());
      }
    }
  } catch {
