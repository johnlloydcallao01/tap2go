...
// Keep same exported types for page compatibility (BFF now returns these shapes)
...
/**
 * BFF thin consumer: single aggregation endpoint.
 * Frontend: get signed-in user on server (via cookie/JWT), call one prepared BFF.
 * Backend (apps/cms/src/app/api/admin/profile) owns: user lookup, admin join, user-events,
 * sanitization, and overrideAccess.
 */
export async function getProfileData(): Promise<{
  ...
  // Call BFF aggregation endpoint (backend is source of truth)
  ...
  // BFF returns { user, raw, admin, activities } already sanitized
  const user = data.user as unknown as User;
  const raw = data.raw as unknown as RawUser;
  const admin = (data.admin as unknown as AdminRecord) || null;
  ...
  return { user, raw: raw || (user as RawUser), admin, activities };
}

/**
 * BFF thin consumer for profile update.
 * Backend owns validation + whitelisting + Payload update with overrideAccess.
 */
export async function updateProfileAction(payloadInput: ProfileUpdateInput): Promise<{
  ...
  // Front-side minimal validation (backend does authoritative validation)
  ...
  const data = await readJson(res);
  if (!res.ok) {
    const msg = extractError(data, 'Failed to update profile');
    const errors = (data as any)?.details || (data as any)?.error;
    // Prefer BFF error message
...
/**
 * BFF thin consumer for password change.
 * Backend owns policy check + current password verification via payload.login + update.
 */
...
/**
 * BFF thin consumer for avatar upload.
 * Backend owns: file validation, media creation, user linking (single transaction).
 */
export async function uploadAvatarAction(formData: FormData): Promise<{
  ...
  // Frontend quick guard (backend does authoritative check)
  ...
  // Forward as multipart to BFF (BFF is safe boundary with overrideAccess)
...
/**
 * BFF thin consumer for avatar removal.
 */
