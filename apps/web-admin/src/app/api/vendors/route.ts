...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/vendors?${searchParams.toString()}`, {
    ...
    const cacheStatus = res.headers.get('X-Vendors-Cache')
    if (cacheStatus) headers.set('X-Vendors-Cache', cacheStatus)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/vendors`, {
      method: 'POST',
