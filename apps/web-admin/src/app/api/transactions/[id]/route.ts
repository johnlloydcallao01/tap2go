...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/transactions/${id}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.text()
    const headers = new Headers({ 'Content-Type': res.headers.get('content-type') || 'application/json' })
    const cacheStatus = res.headers.get('X-Transactions-Cache')
    if (cacheStatus) headers.set('X-Transactions-Cache', cacheStatus)
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/transactions/${id}${qs}`, {
      method: 'DELETE',
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
