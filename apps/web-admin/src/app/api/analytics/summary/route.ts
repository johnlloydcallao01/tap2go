...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/analytics/summary?${forward.toString()}`, {
    ...
    if (!res.ok) {
      await res.text()
      return NextResponse.json({ error: 'Failed to load analytics summary' }, { status: res.status })
    }
