...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/reports/overview?${forward.toString()}`, {
    ...
    if (!res.ok) {
      await res.text()
      return NextResponse.json({ error: 'Failed to load reports overview' }, { status: res.status })
    }
