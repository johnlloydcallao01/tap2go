...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/configuration?${searchParams.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.text()
    const headers = new Headers({ 'Content-Type': res.headers.get('content-type') || 'application/json' })
    const cacheStatus = res.headers.get('X-Configuration-Cache')
    if (cacheStatus) headers.set('X-Configuration-Cache', cacheStatus)
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/configuration`, {
      method: 'PATCH',
