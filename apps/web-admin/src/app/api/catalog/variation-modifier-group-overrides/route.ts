...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/catalog/variation-modifier-group-overrides?${searchParams.toString()}`, {
...
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/catalog/variation-modifier-group-overrides`, {
      method: 'POST',
