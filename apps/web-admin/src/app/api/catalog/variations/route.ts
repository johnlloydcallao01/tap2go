...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/catalog/variations?${searchParams.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.text()
    const headers = new Headers({ 'Content-Type': res.headers.get('content-type') || 'application/json' })
    const cacheStatus = res.headers.get('X-Variations-Cache')
    if (cacheStatus) headers.set('X-Variations-Cache', cacheStatus)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/catalog/variations`, {
      method: 'POST',
