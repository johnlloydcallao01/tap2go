...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/catalog/merchant-variation-modifier-option-overrides/${id}`, {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/catalog/merchant-variation-modifier-option-overrides/${id}${qs}`, {
      method: 'DELETE',
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
