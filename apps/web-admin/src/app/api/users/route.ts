...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/users?${searchParams.toString()}`, {
    ...
    const cacheStatus = res.headers.get('X-Users-Cache')
    if (cacheStatus) headers.set('X-Users-Cache', cacheStatus)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/users`, {
      method: 'POST',
