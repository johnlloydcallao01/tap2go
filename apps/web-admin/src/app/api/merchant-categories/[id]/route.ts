...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/merchant-categories/${id}`, { headers: { Authorization: `JWT ${token}` }, cache: 'no-store' })
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/merchant-categories/${id}`, {
      method: 'PATCH',
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!token) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  try {
    const res = await fetch(`${CMS_BASE}/admin/merchant-categories/${id}`, { method: 'DELETE', headers: { Authorization: `JWT ${token}` }, cache: 'no-store' })
    const data = await res.text()
    return new NextResponse(data, { status: res.status, headers: { 'Content-Type': res.headers.get('content-type') || 'application/json' } })
  } catch { return NextResponse.json({ error: 'Failed to reach CMS' }, { status: 502 }) }
}
