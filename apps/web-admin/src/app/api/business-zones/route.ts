...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/business-zones?${searchParams.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.text()
    const cacheHeader = res.headers.get('x-businesszones-cache')
    return new NextResponse(data, {
      status: res.status,
      headers: {
        'Content-Type': res.headers.get('content-type') || 'application/json',
        'Cache-Control': 'private, max-age=30, stale-while-revalidate=60',
        ...(cacheHeader ? { 'X-BusinessZones-Cache': cacheHeader } : {}),
      },
    })
...
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/business-zones`, {
      method: 'POST',
