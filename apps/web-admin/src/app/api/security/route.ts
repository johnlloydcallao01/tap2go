...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/security?${searchParams.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.text()
    const headers = new Headers({ 'Content-Type': res.headers.get('content-type') || 'application/json' })
    const cacheStatus = res.headers.get('X-Security-Cache')
    if (cacheStatus) headers.set('X-Security-Cache', cacheStatus)
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/admin/security`, {
      method: 'PATCH',
