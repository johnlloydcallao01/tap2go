/**
 * @file apps/cms/src/app/api/admin/profile/password/route.ts
 * @description BFF for password change. Backend owns verification and policy.
 * POST /api/admin/profile/password  body: { userId, currentPassword, newPassword }
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Policy: 8-40, upper, number, special
    ...
    // Resolve user email for login verification
    ...
    // Verify current password via payload.login (overrideAccess safe)
    try {
      await payload.login({
        collection: 'users',
