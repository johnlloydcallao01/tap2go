/**
 * @file apps/cms/src/app/api/admin/catalog/merchant-product-modifier-option-overrides/[id]/route.ts
 */
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    try { doc = await payload.findByID({ collection: 'merchant-product-modifier-option-overrides', id: docId as number, depth: 2, overrideAccess: true }) as unknown as Record<string, any> } catch (e:any){ return NextResponse.json({ error: 'Merchant product modifier option override not found', details: e?.message }, { status: 404 }) }
    if (!doc) return NextResponse.json({ error: 'Merchant product modifier option override not found' }, { status: 404 })
    const sanitized = sanitizeDoc(doc)
    return NextResponse.json({ doc: sanitized })
  } catch (err:any){ console.error('[admin/catalog/merchant-product-modifier-option-overrides/[id]] GET error:', err); return NextResponse.json({ error: err?.message||'Failed to load merchant product modifier option override' }, { status: 500 })}
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    try { existing = await payload.findByID({ collection: 'merchant-product-modifier-option-overrides', id: docId as number, depth: 0, overrideAccess: true }) as unknown as Record<string, any> } catch (e:any){ return NextResponse.json({ error: 'Merchant product modifier option override not found' }, { status: 404 }) }
    ...
    if (body.merchant_product_id !== undefined || body.merchantProductId !== undefined || body.merchant_product !== undefined) {
      ...
      const mpId=Number(raw); if(Number.isNaN(mpId)) return NextResponse.json({ error: 'merchant_product_id must be numeric' }, { status: 400 })
    ...
    if (mergedMerchantProductId != null && mergedOptionId != null) {
    ...
    try { updated = await payload.update({ collection: 'merchant-product-modifier-option-overrides', id: docId as number, data: patch as any, depth: 2, overrideAccess: true }) as unknown as Record<string, any> } catch(e:any){ const msg=e?.message||'Failed to update merchant product modifier option override'; const lower=String(msg).toLowerCase(); if(lower.includes('unique')||lower.includes('duplicate')) return NextResponse.json({ error: 'Duplicate merchant_product_id + base_modifier_option_id combination', details: msg }, { status:409 }); return NextResponse.json({ error: msg, details: e?.data||e?.errors }, { status:400 }) }
    const sanitized=sanitizeDoc(updated)
    // Bust list cache so the update reflects immediately on /catalog/merchant-product-modifier-option-overrides
    await deleteCachedByPrefix('admin:catalog-merchant-product-modifier-option-overrides:')
    return NextResponse.json({ success: true, message: 'Merchant product modifier option override updated successfully', doc: sanitized })
  } catch(err:any){ console.error('[admin/catalog/merchant-product-modifier-option-overrides/[id]] PATCH error:', err); return NextResponse.json({ error: err?.message||'Update failed' }, { status:500 }) }
}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const docId: number | string = Number.isFinite(numericId) ? numericId : id
    let deleted:any
    try { deleted = await payload.delete({ collection: 'merchant-product-modifier-option-overrides', id: docId as number, overrideAccess: true }) } catch(e:any){ return NextResponse.json({ error: e?.message||'Failed to delete merchant product modifier option override' }, { status:400 }) }
    if(!deleted) return NextResponse.json({ error: 'Merchant product modifier option override not found' }, { status:404 })
    // Bust list cache so the deletion reflects immediately on /catalog/merchant-product-modifier-option-overrides
    await deleteCachedByPrefix('admin:catalog-merchant-product-modifier-option-overrides:')
    return NextResponse.json({ success: true, id: deleted.id, message: 'Merchant product modifier option override deleted successfully' })
  } catch(err:any){ console.error('[admin/catalog/merchant-product-modifier-option-overrides/[id]] DELETE error:', err); return NextResponse.json({ error: err?.message||'Delete failed' }, { status:500 }) }
}
