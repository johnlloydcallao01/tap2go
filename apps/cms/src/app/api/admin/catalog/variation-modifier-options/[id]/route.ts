...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Bust list cache so the update reflects immediately on /catalog/variation-modifier-options
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Bust list cache so the deletion reflects immediately on /catalog/variation-modifier-options
    await deleteCachedByPrefix('admin:catalog-variation-modifier-options:')
