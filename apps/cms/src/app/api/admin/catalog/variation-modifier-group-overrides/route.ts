...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `admin:catalog-variation-modifier-group-overrides:${admin.id}:${cacheQuery}`
    const cached = await getCached<Record<string, unknown>>(cacheKey)
    if (cached) return NextResponse.json(cached, { headers: { 'X-VariationModifierGroupOverrides-Cache': 'HIT' } })
    ...
    return NextResponse.json(responseBody, { headers: { 'X-VariationModifierGroupOverrides-Cache': 'MISS' } })
  } catch (err: any) {
