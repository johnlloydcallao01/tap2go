...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const cacheKey = `admin:catalog-attributes:detail:v1:${String(docId)}`
    const { data, status } = await getOrBuildDashboard(cacheKey, 120, () =>
      withAdminRequestSlot(async () => {
        ...
        try {
          ...
          throw Object.assign(new Error('Attribute not found'), { status: 404, details: e?.message })
        }
        if (!doc) throw Object.assign(new Error('Attribute not found'), { status: 404 })
    ...
    if (err instanceof Error && (err as unknown as { status?: number }).status === 404) {
      return NextResponse.json({ error: 'Attribute not found', details: (err as unknown as { details?: unknown }).details }, { status: 404 })
    }
