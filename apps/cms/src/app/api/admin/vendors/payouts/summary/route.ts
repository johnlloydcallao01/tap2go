...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = `admin:payouts:summary:${admin.id}:${buildPayoutsCacheQuery(searchParams)}`
    ...
    const response = {
      ...
      summary: toPayoutSummary(rows),
      verificationBreakdown: buildVerificationBreakdown(core),
    }
