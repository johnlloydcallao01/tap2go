/**
 * @file apps/cms/src/app/api/vendor/activity/views/route.ts
 * @description BFF aggregation endpoint for web-merchant /activity/views (vendor-scoped, read-only).
 ...
 *
 * GET /api/vendor/activity/views?userId=&page=&limit=&search=&outletId=&itemType=&sort=
 *     -> { docs, pagination, stats, meta }
 *
 * Merchant differences vs admin BFF (admin/customer-activity/views): scope is limited to
 * rows touching the vendor's own outlets (itemType=merchant AND merchant in vendor's
 * merchants) or the vendor's own listings (itemType=merchant_product AND merchantProduct
 * in vendor's merchant-products). Note recent-views uses snake `merchant_product` for
 * the itemType value (unlike wishlists' camel `merchantProduct`). Read-only. PII is
 * minimal: id/email/firstName/lastName only — no phone, birthDate, addresses, or views
...
const ITEM_TYPES = new Set(['merchant', 'merchant_product'])
const ALLOWED_SORT = new Set(['-lastViewedAt', 'lastViewedAt', '-viewCount', 'viewCount', '-createdAt', 'createdAt', '-updatedAt', 'updatedAt'])
...
export async function GET(request: NextRequest) {
  try {
    ...
    const emptyOut = (page: number, limit: number, stats: Record<string, any> | null = null) => ({
      ...
      stats,
      meta: { generatedAt: new Date().toISOString(), sort: '-lastViewedAt', search: '' },
    })
    ...
    let sort = (searchParams.get('sort') || '-lastViewedAt').trim()
    if (!ALLOWED_SORT.has(sort)) sort = '-lastViewedAt'
    ...
    // Accept both snake (stored) and camel (client) spellings.
    const typeFilter = typeCsv
      .map((v) => (v === 'merchantproduct' ? 'merchant_product' : v))
      .filter((v) => ITEM_TYPES.has(v))

    // 1. Vendor's listings (for merchant_product-scoped views).
    ...
    for (const mp of mpDocs) {
      ...
      mpTitleById.set(mpid, str((mp as any).display_title, `Listing #${mpid}`))
    ...
    // 3. Views scoped to the vendor's outlets/listings (depth 2 populates user + merchant + merchantProduct).
    const viewsRes = await payload.find({
      collection: 'recent-views',
      where: {
        or: [
          { and: [{ itemType: { equals: 'merchant' } }, { merchant: { in: outletIds } }] },
          { and: [{ itemType: { equals: 'merchant_product' } }, { merchantProduct: { in: Array.from(scopedMpIds) } }] },
        ],
      } as any,
      limit: 5000,
      depth: 2,
    ...
    let viewDocs = (((viewsRes as any).docs as Record<string, any>[]) ?? [])

    let enriched = viewDocs.map((w) => {
      ...
      // Listing's outlet wins for merchant_product views (the listing belongs to exactly one outlet).
      ...
      const directPid = relId(w.product)
      const resolvedPid = pid ?? directPid ?? null
      const pinfo = resolvedPid != null ? productNameById.get(resolvedPid) : undefined
      const mpTitle = mpid != null ? (mpTitleById.get(mpid) ?? (mpRaw ? str((mpRaw as any).display_title, '') : '')) : ''
      return {
        raw: w,
        itemType,
        ...
        product: resolvedPid != null ? { id: resolvedPid, name: pinfo?.name || `Product #${resolvedPid}`, slug: pinfo?.slug || '' } : null,
        viewCount: num(w.viewCount, 0),
        lastViewedAt: w.lastViewedAt ? String(w.lastViewedAt) : null,
        firstViewedAt: w.firstViewedAt ? String(w.firstViewedAt) : null,
        source: str(w.source, 'unknown'),
      }
    ...
    switch (sort) {
      case 'lastViewedAt':
        enriched.sort((a, b) => (a.lastViewedAt || '').localeCompare(b.lastViewedAt || ''))
        break
      case '-viewCount':
        enriched.sort((a, b) => b.viewCount - a.viewCount || (b.lastViewedAt || '').localeCompare(a.lastViewedAt || ''))
        break
      case 'viewCount':
        enriched.sort((a, b) => a.viewCount - b.viewCount || (b.lastViewedAt || '').localeCompare(a.lastViewedAt || ''))
        break
      case '-createdAt':
      ...
      case '-lastViewedAt':
      default:
        enriched.sort((a, b) => (b.lastViewedAt || '').localeCompare(a.lastViewedAt || ''))
        break
    }
    ...
    const itemTypeSplit: Record<string, number> = { merchant: 0, merchant_product: 0 }
    const perOutlet = new Map<number, { id: number; outletName: string; count: number; views: number }>()
    for (const e of enriched) {
      if (e.customer) uniqueCustomers.add(e.customer.id)
      if (e.product) uniqueProducts.add(e.product.id)
      totalViewCount += e.viewCount
      const t = e.itemType === 'merchant_product' ? 'merchant_product' : 'merchant'
      itemTypeSplit[t] = (itemTypeSplit[t] || 0) + 1
      if (e.outletId != null) {
        const prev = perOutlet.get(e.outletId) || { id: e.outletId, outletName: e.merchant?.outletName || `Outlet #${e.outletId}`, count: 0, views: 0 }
        prev.count += 1
        prev.views += e.viewCount
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        totalViewCount,
        itemTypeSplit,
        perOutlet: Array.from(perOutlet.values()).sort((a, b) => b.views - a.views),
      },
      meta: { generatedAt: new Date().toISOString(), sort, search: searchParams.get('search')?.trim() || '' },
    })
  } catch (err: any) {
    console.error('[vendor/activity/views] GET error:', err)
    return NextResponse.json({ error: err?.message || 'Failed to load recently viewed' }, { status: 500 })
  }
}
