/**
 * @file apps/cms/src/app/api/vendor/catalog/merchant-variation-modifier-group-overrides/route.ts
 * @description Read-only BFF aggregation for web-merchant /catalog/merchant-variation-modifier-group-overrides.
 *
 * Divergence from admin (cms/.../admin/catalog/merchant-variation-modifier-group-overrides): this override
 * collection keys on merchant_product_id + variation_id (MerchantVariationModifierGroupOverrides.ts:31-62)
 * and branches by target_group_source into base_modifier_group_id (modifier-groups) or
 * variation_modifier_group_id (variation-modifier-groups). Merchant scope derives via
...
export async function GET(request: NextRequest) {
  try {
    ...
    const variationGroupParam = (searchParams.get('variation_modifier_group_id') || searchParams.get('variationModifierGroupId') || searchParams.get('variation_group_id') || '').trim()
    ...
    if (merchantIds.length === 0) {
      return NextResponse.json({ docs: [], merchant_products: [], variations: [], base_groups: [], variation_groups: [], stats: null, pagination: { page, limit, totalDocs: 0, totalPages: 0, hasNextPage: false, hasPrevPage: false } })
    }
    ...
    // 3. Owned products -> owned variations + owned modifier-groups (base group candidates).
    ...
    const base_groups = ownedModifierGroupDocs
    ...
    // 4. Owned variation-modifier-groups (variation-added group candidates).
    ...
    const ownedVariationGroupIds = new Set(ownedVariationGroupDocs.map((g) => Number(g.id)).filter((v) => Number.isFinite(v)))

    const variation_groups = ownedVariationGroupDocs
      .map((g) => ({ id: Number(g.id), name: str(g.name, ''), variation_id: extractId(g.variation_id) }))
      .sort((a, b) => a.name.localeCompare(b.name))
    ...
    if (variationGroupParam) {
      ...
      if (!Number.isFinite(gid)) return NextResponse.json({ error: 'variationModifierGroupId must be numeric' }, { status: 400 })
      if (!ownedVariationGroupIds.has(gid)) {
        return NextResponse.json({ error: 'Forbidden: variation modifier group does not belong to vendor' }, { status: 403 })
      }
    }

    // 6. Overrides scoped to owned merchant products.
    ...
    if (variationGroupParam) where.variation_modifier_group_id = { equals: Number(variationGroupParam) }
    if (modeCsv.length) {
    ...
    const [paginated, statsAll] = await Promise.all([
      ...
      payload.find({ collection: 'merchant-variation-modifier-group-overrides', where: finalWhere, limit: 0, depth: 0, overrideAccess: true }),
    ])
    ...
    return NextResponse.json({
      ...
      base_groups,
      variation_groups,
      stats: { total, filteredTotal, modeBreakdown, sourceBreakdown, requiredBehaviorBreakdown, activeCount, inactiveCount },
      pagination: {
        page: paginated.page,
    ...
    const msg = err instanceof Error ? err.message : 'Failed to fetch merchant variation modifier group overrides'
    console.error('[vendor/catalog/merchant-variation-modifier-group-overrides] GET error:', err)
