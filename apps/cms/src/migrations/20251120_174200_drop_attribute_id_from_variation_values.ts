...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.execute(sql`DROP INDEX IF EXISTS prod_variation_values_attribute_id_idx;`)

  await db.execute(sql`
    DO $$ BEGIN
      ALTER TABLE prod_variation_values
      DROP CONSTRAINT IF EXISTS prod_variation_values_attribute_id_id_prod_attributes_id_fk;
    EXCEPTION WHEN undefined_object THEN NULL; END $$;
  `)
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  await db.execute(sql`
    DO $$ BEGIN
      ALTER TABLE prod_variation_values
      ADD CONSTRAINT prod_variation_values_attribute_id_id_prod_attributes_id_fk
