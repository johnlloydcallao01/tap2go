...
export class CMSStats {
  ...
  static async getStatusCounts(contentType: 'posts' | 'pages') {
    ...
    const items = data || [];
    return {
      all: items.filter((item: { status: string; deleted_at?: string }) => !item.deleted_at).length,
      published: items.filter((item: { status: string; deleted_at?: string }) => item.status === 'published' && !item.deleted_at).length,
      draft: items.filter((item: { status: string; deleted_at?: string }) => item.status === 'draft' && !item.deleted_at).length,
      trash: items.filter((item: { status: string; deleted_at?: string }) => item.deleted_at).length
    };
  }
}
