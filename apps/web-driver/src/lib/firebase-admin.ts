...
function initializeFirebaseAdmin() {
  try {
    ...
    if (!projectId || !privateKey || !clientEmail) {
      // During build time, don't throw errors to prevent build failures
      if (process.env.NODE_ENV === 'production' && process.env.CI) {
        console.warn('Firebase Admin environment variables not found during build. Admin features will be disabled at runtime.');
        return;
      }
...
// Export services with multiple export patterns for compatibility
export { adminDb, adminAuth };
export { adminAuth as auth }; // Alternative export name
export { adminDb as firestore }; // Alternative export name
