...
// Lazy-loaded Bonsai client to avoid build-time errors
let _bonsaiClient: Client | null = null;

function createBonsaiClient(): Client {
...
export const bonsaiClient = (): Client => {
  if (!_bonsaiClient) {
    _bonsaiClient = createBonsaiClient();
  }
  return _bonsaiClient;
};
