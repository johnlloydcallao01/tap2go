...
// Lazy-loaded client instances to avoid build-time errors
let _paymongoPublicClient: AxiosInstance | null = null;
let _paymongoSecretClient: AxiosInstance | null = null;

export const paymongoPublicClient = (): AxiosInstance => {
  if (!_paymongoPublicClient) {
    _paymongoPublicClient = createPayMongoClient(false);
  }
  return _paymongoPublicClient;
};

export const paymongoSecretClient = (): AxiosInstance => {
  if (!_paymongoSecretClient) {
    _paymongoSecretClient = createPayMongoClient(true);
  }
  return _paymongoSecretClient;
};
