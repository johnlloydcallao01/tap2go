...
export interface MerchantNotificationsResult {
  docs: MerchantNotification[]
  totalDocs: number
  unreadCount: number
  unseenCount: number
}
...
function mapNotification(raw: Record<string, unknown>): MerchantNotification | null {
...
export async function getMyNotifications(): Promise<MerchantNotificationsResult | null> {
  ...
  const docs = (data.docs || []).map(mapNotification).filter((item): item is MerchantNotification => Boolean(item && !item.archivedAt))
  return {
    docs,
...
export async function updateMyNotification(
  id: number | string,
  data: Partial<Pick<MerchantNotification, 'status' | 'seenAt' | 'readAt'>>,
) {
  const user = await getServerUser()
  if (!user || user.role !== 'vendor') return null
...
export async function markAllMyNotifications(data: { status?: 'read'; seenAt?: string }) {
  ...
  await Promise.all(result.docs.map((item) => updateMyNotification(item.id, data)))
  return true
}
