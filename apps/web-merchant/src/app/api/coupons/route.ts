...
// GET /api/coupons -> vendor-scoped coupon list via CMS vendor BFF.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'page', 'limit', 'status', 'discount_type']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/coupons?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch coupons' }, { status: res.status })
...
// POST /api/coupons -> create a coupon for the caller's own brand.
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${CMS_BASE}/vendor/coupons`, {
      method: 'POST',
      headers: {
    ...
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to create coupon', details: data.details }, { status: res.status })
