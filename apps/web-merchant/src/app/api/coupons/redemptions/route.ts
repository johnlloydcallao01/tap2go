...
// GET /api/coupons/redemptions -> vendor-scoped coupon usage report via CMS vendor BFF.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['couponId', 'orderId', 'customerId', 'status', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/coupons/redemptions?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch coupon usage' }, { status: res.status })
