...
// GET /api/orders -> vendor-scoped read-only orders across the vendor's merchants.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'page', 'limit', 'status', 'fulfillment_type', 'fulfillmentType', 'delivery_status', 'deliveryStatus', 'has_discount']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/orders?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch orders' }, { status: res.status })
