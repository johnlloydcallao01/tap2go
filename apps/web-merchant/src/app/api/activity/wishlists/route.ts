...
// GET /api/activity/wishlists -> vendor-scoped wishlist activity via CMS vendor BFF (read-only).
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${CMS_BASE}/vendor/activity/wishlists?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch wishlists' }, { status: res.status })
