...
export async function GET(request: NextRequest){
  ...
  try{
    ...
    const res=await fetch(`${CMS_BASE}/vendor/reports/catalog?${forward.toString()}`,{headers:{Authorization:`JWT ${token}`}, cache:'no-store', signal:AbortSignal.timeout(25000)})
