...
// GET /api/customers -> vendor-scoped customer list via CMS vendor BFF (read-only).
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${CMS_BASE}/vendor/customers?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch customers' }, { status: res.status })
