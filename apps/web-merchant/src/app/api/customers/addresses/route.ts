...
// GET /api/customers/addresses -> vendor-scoped customer delivery addresses via CMS vendor BFF (read-only).
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'page', 'limit', 'outletId', 'merchantId', 'address_type', 'addressType', 'is_default', 'isDefault']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/customers/addresses?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch customer addresses' }, { status: res.status })
