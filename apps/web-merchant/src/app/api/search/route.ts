...
function getUnpopulatedProductId(doc: PayloadDoc): string | number | null {
  const raw = (doc.product_id ?? doc.product) as unknown;
...
function getUnpopulatedMediaId(doc: PayloadDoc): string | number | null {
  ...
  if (typeof primary === 'string' && /^\d+$/.test(primary)) return primary;
  return null;
}
