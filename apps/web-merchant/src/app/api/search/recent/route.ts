...
async function forward(request: NextRequest, method: 'GET' | 'POST' | 'DELETE') {
  ...
  const init: RequestInit = { method, headers: { Authorization: `JWT ${token}`, 'Content-Type': 'application/json' }, cache: 'no-store' }
  if (method === 'POST') init.body = await request.text()
  const response = await fetch(`${CMS_BASE}/vendor/search/recent${url.search}`, init)
