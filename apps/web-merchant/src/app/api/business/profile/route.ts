...
// GET /api/business/profile -> Company Information (read-only).
// Thin proxy over CMS GET /api/vendor/profile?userId=<vendorUserId> (BFF aggregation:
// user + vendor + merchants summary + activities, overrideAccess internally).
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${CMS_BASE}/vendor/profile?userId=${user.id}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch business profile' }, { status: res.status })
