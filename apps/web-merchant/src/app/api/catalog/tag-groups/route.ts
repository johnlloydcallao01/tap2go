...
// GET /api/catalog/tag-groups -> vendor-scoped read-only tag groups.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'page', 'limit', 'is_active', 'isActive', 'is_filterable', 'isFilterable', 'is_searchable', 'isSearchable']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/tag-groups?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch tag groups' }, { status: res.status })
