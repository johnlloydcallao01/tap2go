...
// GET /api/catalog/tags -> vendor-scoped read-only product tags.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'page', 'limit', 'is_active', 'isActive', 'is_featured', 'isFeatured', 'tag_type', 'tagType', 'parent_tag_id', 'parent']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/tags?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch tags' }, { status: res.status })
