...
// GET /api/catalog/variation-modifier-option-overrides -> vendor-scoped read-only variation modifier option overrides.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'variationId', 'variation_id', 'base_modifier_option_id', 'baseModifierOptionId', 'base_option_id', 'mode', 'default_behavior', 'defaultBehavior', 'availability_behavior', 'availabilityBehavior', 'is_active', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/variation-modifier-option-overrides?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch variation modifier option overrides' }, { status: res.status })
