...
// GET /api/catalog/variations -> vendor-scoped read-only variations.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'productId', 'product_id', 'modifier_behavior_mode', 'mode', 'is_visible', 'is_used', 'is_used_for_variations', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/variations?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch variations' }, { status: res.status })
