...
// GET /api/catalog/merchant-variation-modifier-group-overrides -> vendor-scoped read-only merchant variation modifier group overrides.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'merchant_product_id', 'merchantProductId', 'variation_id', 'variationId', 'base_modifier_group_id', 'baseModifierGroupId', 'base_group_id', 'variation_modifier_group_id', 'variationModifierGroupId', 'variation_group_id', 'target_group_source', 'targetGroupSource', 'mode', 'required_behavior', 'requiredBehavior', 'selection_type_override', 'selectionTypeOverride', 'is_active', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/merchant-variation-modifier-group-overrides?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch merchant variation modifier group overrides' }, { status: res.status })
