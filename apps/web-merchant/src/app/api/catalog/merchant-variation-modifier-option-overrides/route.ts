...
// GET /api/catalog/merchant-variation-modifier-option-overrides -> vendor-scoped read-only merchant variation modifier option overrides.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'merchant_product_id', 'merchantProductId', 'variation_id', 'variationId', 'base_modifier_option_id', 'baseModifierOptionId', 'base_option_id', 'variation_modifier_option_id', 'variationModifierOptionId', 'variation_option_id', 'target_option_source', 'targetOptionSource', 'mode', 'default_behavior', 'defaultBehavior', 'availability_behavior', 'availabilityBehavior', 'is_active', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/merchant-variation-modifier-option-overrides?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch merchant variation modifier option overrides' }, { status: res.status })
