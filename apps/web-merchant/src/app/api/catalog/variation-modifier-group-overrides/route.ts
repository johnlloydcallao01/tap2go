...
// GET /api/catalog/variation-modifier-group-overrides -> vendor-scoped read-only variation modifier group overrides.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'variationId', 'variation_id', 'base_modifier_group_id', 'baseModifierGroupId', 'base_group_id', 'mode', 'required_behavior', 'requiredBehavior', 'selection_type_override', 'selectionTypeOverride', 'is_active', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/variation-modifier-group-overrides?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch variation modifier group overrides' }, { status: res.status })
