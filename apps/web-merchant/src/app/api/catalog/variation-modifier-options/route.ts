...
// GET /api/catalog/variation-modifier-options -> vendor-scoped read-only variation modifier options.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'groupId', 'variation_modifier_group_id', 'variationModifierGroupId', 'is_available', 'is_default', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/variation-modifier-options?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch variation modifier options' }, { status: res.status })
