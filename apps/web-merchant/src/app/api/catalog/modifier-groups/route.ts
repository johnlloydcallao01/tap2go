...
// GET /api/catalog/modifier-groups -> vendor-scoped read-only modifier groups.
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const k of ['search', 'sort', 'productId', 'product_id', 'selection_type', 'is_required', 'page', 'limit']) {
      const v = searchParams.get(k)
      if (v != null && v !== '') qs.set(k, v)
    }
    const res = await fetch(`${CMS_BASE}/vendor/catalog/modifier-groups?${qs.toString()}`, {
      headers: { Authorization: `JWT ${token}` },
      cache: 'no-store',
    })
    const data = await res.json()
    if (!res.ok) return NextResponse.json({ error: data.error || 'Failed to fetch modifier groups' }, { status: res.status })
